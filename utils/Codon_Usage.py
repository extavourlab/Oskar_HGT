...
class CodonAdaptationIndex(object):
    ...
    def generate_index(self, fasta_file):
        ...
        if self.index != {} or self.codon_count != {}:
            raise ValueError(
                "an index has already been set or a codon count has been done. cannot overwrite either.")
        ...
        for aa in SynonymousCodons:
            total = 0.0
            # RCSU values are CodonCount/((1/num of synonymous codons) * sum of all
            # synonymous codons)
            rcsu = []
            ...
            rcsu_max = max(rcsu)
            self.RCSU['max_' + aa] = rcsu_max
            for i in range(len(codons)):
                self.index[codons[i]] = rcsu[i] / rcsu_max
                self.RCSU[codons[i]] = rcsu[i]
        return self.RCSU, self.index
    ...
    def cai_for_gene(self, dna_sequence):
        ...
        if self.index == {}:
            raise ValueError("No Index stored")
    ...
    def _count_codons(self, fasta_file):
        with open(fasta_file, 'r') as handle:
            ...
            for cur_record in SeqIO.parse(handle, "fasta"):
                ...
                if cur_record.seq[0:3] == 'ATG' and len(cur_record.seq) % 3 == 0:
                    ...
                    for i in range(0, len(dna_sequence), 3):
                        ...
                        if codon in self.codon_count:
                            ...
                            print("illegal codon %s in gene: %s" % (codon, cur_record.id))
